...
export function InspectionForm({
  ...
  const isItemOpenCorrection = (item: ChecklistItem) =>
    item.status === 'fail' && item.failResolution === 'needs_correction'

  const isItemPendingCorrection = (item: ChecklistItem) =>
    isItemOpenCorrection(item) && item.reinspectionResult !== 'effective'
  ...
  const isItemEditable = (item: ChecklistItem) => {
    ...
    return editableItemIds.has(item.id) || isItemOpenCorrection(item)
  }
        ...
        const nextItem = {
          ...
          reinspectionDate:
            failResolution === 'needs_correction'
              ...
              : undefined,
          reinspectionResult:
            failResolution === 'needs_correction' ? null : undefined,
        }
  ...
  const handleReinspectionResultChange = (
    id: string,
    reinspectionResult: 'effective' | 'ineffective',
  ) => {
    onChecklistChange(
      checklist.map((item) => {
        if (item.id !== id) {
        ...
        const shouldResetPlan = reinspectionResult === 'ineffective'

        return {
          ...item,
          reinspectionResult,
          correctionPlan: shouldResetPlan ? '' : item.correctionPlan,
          reinspectionDate: shouldResetPlan ? '' : item.reinspectionDate,
        }
      }),
    )

    setToastMessage(
      reinspectionResult === 'effective'
        ? 'Correção marcada como eficaz.'
        : 'Correção marcada como ineficaz. Informe novo plano e data.',
    )
  }
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {isItemPendingCorrection(item) && (
                      <span className="px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wide bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
                        Em Aberto
                      </span>
                    )}
                  </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-col sm:flex-row gap-2">
                            <button
                              type="button"
                              onClick={() =>
                                handleReinspectionResultChange(
                                  item.id,
                                  'effective',
                                )
                              }
                              disabled={!isItemEditable(item)}
                              className={`
                                flex items-center justify-center gap-2 px-3 py-2 rounded-md font-medium transition-all text-sm
                                ${
                                  item.reinspectionResult === 'effective'
                                    ? 'bg-green-600 text-white shadow-md'
                                    : 'bg-gray-100 text-gray-700 hover:bg-green-100 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-green-900/30'
                                }
                                disabled:opacity-40 disabled:cursor-not-allowed
                              `}
                            >
                              <CheckCircle size={16} /> Correção Eficaz
                            </button>
                            <button
                              type="button"
                              onClick={() =>
                              ...
                              `}
                            >
                              <XCircle size={16} /> Correção Ineficaz
                            </button>
                          </div>
                        </div>
                      )}
