...
const ROLE_OPTIONS = [
  "Pedreiro", "Servente", "Mestre de Obras", "Engenheiro", "Técnico de Segurança", "Encarregado", "Auxiliar",
];
...
const CHECKLIST_PRESETS = [
  ...
  { category: "Acabamento", description: "Verificação de reboco e regularidade" },
...
export function InspectionForm({
  ...
  const handleAddTeamMember = () => {
    if (newMember.name && newMember.role) {
      onTeamChange([...team, { id: crypto.randomUUID(), name: newMember.name, role: newMember.role }]);
  ...
  const handleStatusChange = (id: string, status: 'pass' | 'fail' | 'na') => {
    onChecklistChange(checklist.map((item) => item.id === id ? { ...item, status } : item));
  };
          ...
          <div className="space-y-2">
            {team.map((member) => (
                <div key={member.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-md">
                  ...
                  <button onClick={() => handleRemoveTeamMember(member.id)} className="p-2 text-red-600 hover:bg-red-50 dark:text-red-400 rounded-md"><Trash2 size={18} /></button>
                </div>
            ))}
          </div>
          <div className="space-y-3 pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <select value={newMember.name} onChange={handleMemberSelect} className={INPUT_CLASS}>
                <option value="">Selecione o integrante...</option>
                {PREDEFINED_MEMBERS.map((m) => (<option key={m.name} value={m.name}>{m.name}</option>))}
              </select>
              <select value={newMember.role} onChange={(e) => setNewMember({ ...newMember, role: e.target.value })} className={INPUT_CLASS}>
                <option value="">Selecione a função...</option>
                {ROLE_OPTIONS.map((r) => (<option key={r} value={r}>{r}</option>))}
              </select>
            </div>
            <button onClick={handleAddTeamMember} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md transition-colors"><Plus size={18} /> Adicionar Integrante ao PDF</button>
          </div>
        </div>
            ...
            {checklist.map((item) => (
                <div key={item.id} className="p-4 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg space-y-3 shadow-sm">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <button onClick={() => handleStatusChange(item.id, 'pass')} className={`flex items-center justify-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-all ${item.status === 'pass' ? "bg-green-600 text-white shadow-md ring-2 ring-green-600" : "bg-white dark:bg-gray-700 border border-gray-200"}`}><CheckCircle size={16} /> <span className="hidden sm:inline">Aprovado</span></button>
                    <button onClick={() => handleStatusChange(item.id, 'fail')} className={`flex items-center justify-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-all ${item.status === 'fail' ? "bg-red-600 text-white shadow-md ring-2 ring-red-600" : "bg-white dark:bg-gray-700 border border-gray-200"}`}><XCircle size={16} /> <span className="hidden sm:inline">Reprovado</span></button>
                    <button onClick={() => handleStatusChange(item.id, 'na')} className={`flex items-center justify-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-all ${item.status === 'na' ? "bg-gray-600 text-white shadow-md ring-2 ring-gray-600" : "bg-white dark:bg-gray-700 border border-gray-200"}`}><MinusCircle size={16} /> <span className="hidden sm:inline">N/A</span></button>
                  </div>
                </div>
            ))}
          </div>
      ...
      ======================== */}
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea {...register('observations')} rows={6} className={INPUT_CLASS} placeholder="Digite quaisquer observações relevantes sobre a inspeção..." />
          </div>
        </div>
